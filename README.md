...
Генерирует лендинг по описанию. Описал продукт - получил готовую страницу с текстами и дизайном, публикуется по ссылке.

Стек: next.js 16, typescript, tailwind, prisma (sqlite, для прода postgres), nextauth.
...
В .env нужны `AUTH_SECRET` (openssl rand -base64 32) и `ANTHROPIC_API_KEY`, шаблон в .env.example. Для уведомлений о заявках — `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID` (опционально).

Что умеет:

- регистрация и вход по почте (rate limit на попытки)
- дашборд со списком лендингов, миниатюрами и просмотрами
- шаблоны: 6 готовых пресетов (кофейня, курс, SaaS, фитнес, бьюти, агентство)
- билдер: описание, стиль (5 штук), секции (9), доп. требования
- генерация по описанию, можно перегенерировать или править HTML руками
- свои стили: кастомный CSS поверх сгенерированного лендинга
- форма заявки на лендинге с уведомлениями в Telegram
- превью в реальном времени
- публикация по ссылке /s/slug с SEO-тегами и sitemap
- экспорт лендинга в zip (index.html + README)
...
2. Генератор собирает HTML лендинга с текстами и дизайном.
...
2. В настройках проекта задай переменные:
   ...
   - `ANTHROPIC_API_KEY` — ключ API для генерации
